...
/**
 * JIT 提权闭环：收到 3002 → 弹窗 → 提权 → 重拉权限 → **重放原请求**。
 *
...
export function useElevationFlow() {
  ...
  const submit = useMutation({
    ...
    onSuccess: async () => {
      ...
      setOpen(false)
      // 先把权限拉新，再重放 —— 反过来的话重放会带着旧快照又吃一次 3002。
      await qc.invalidateQueries({ queryKey: [PERM_KEY] })
      await qc.invalidateQueries({ queryKey: [ELEVATION_KEY] })
      retry?.()
      setRetry(null)
    },
  ...
  const request = useCallback((err: NormalizedError, replay?: () => void) => {
    setPending(err)
    setRetry(() => replay ?? null)
    if (!canElevate) {
  ...
  const dialog = (
    ...
    >
      <Alert
        ...
        description="提权只能激活你【本来就持有】的角色（同 sudo 的原义），并且有时限。整个过程会被审计记录。"
      />
