...
| 变更 | 处理 | 理由 |
|---|---|---|
| **收权**：撤销、部门级授权、角色权限、组织调整、离职 | bump **租户纪元**，各节点 1 秒内作废该租户 | 撤权延迟生效是安全事故，宁可租户内懒重算 |
| **给个人加权** | 只 bump 用户版本 + 本节点剔除 + 通知其它节点 | 通知丢了最坏是新权限晚几分钟生效，**不构成安全问题** |

放弃对称换来的是：**热路径只需比对一个每秒刷新的租户纪元**，
不必为每个请求查一次用户版本 —— 这是 P99 能压到 1 μs 的关键。
L1/L2 均以 `(tenantId, userId)` 为键。Redis Pub/Sub 只负责在事务提交后快速广播
`tenantId + epoch`；消息丢失时，数据库 `tenant_perm_epoch` 仍会在 1 秒内令旧快照失效。
V18 滚动升级期间开启旧全局 epoch 兼容栅栏；全部节点升级后关闭，才获得完整的租户级失效范围。
...
角色的可编辑继承关系保存于 `oa_iam.role_inherit_edge`，`oa_iam.role_inherit` 只保存由直接边重建的
传递闭包。这样管理端能准确回显“管理员直接选了哪些角色”，判权热路径仍可通过闭包一次展开，
无需递归查询，也不会把闭包中的 `distance=1` 误当成可长期维护的权威关系。

创建、复制、启停、删除或修改继承时，写服务在同一事务内获取租户级 PostgreSQL advisory lock，
校验租户归属和环路，重建该租户的 ACTIVE-only 闭包，再推进租户权限纪元。角色权限矩阵也使用
乐观锁 `version` 防止两个管理员相互覆盖，并推进租户纪元。停用角色后，直接边仍保留以便重新启用，
但该角色及经过它的继承路径不会进入判权闭包；删除则同时清理直接权限和出入边。

角色真值、租户 epoch 和 `oa_iam.iam_outbox` 事件在同一事务提交。提交后 Redis Pub/Sub 使各 JVM
快速清 L1；`oa-app` 再把 `iam.role.changed.v1` 从 Outbox 可靠投递到 OA Kafka，用于审计和外部集成。
Kafka 不参与判权正确性，也不会因不可用而回滚已经提交的角色变更。

管理端只允许创建 `CUSTOM` 角色。内置角色不可停用或删除，`SUPER_ADMIN` 的权限和继承矩阵禁止
在线修改；角色编码创建后不可变，删除采用软删除，且存在授权历史时拒绝删除。所有管理端点要求
`oa:iam:admin`，写操作进入统一审计切面。
...
PC 覆盖组织、在线角色管理、主体授权、用户组、ABAC、权限沙盘、知识库和报表；移动端覆盖待办办理、
打卡、通讯录与公告已读。
...
- `USER_GROUP` 是 OA 内部显式成员组，不复用 Casdoor group。`user_group_member` 支持
  `valid_from/valid_to` 与软撤销；快照 TTL 会被最近成员边界压低，组禁用或移出成员推进租户 epoch。
...
- JIT 提权采用本地四眼审批表 `elevation_request`：申请只写 PENDING，另一名权限管理员批准后才生成
  elevation-only 临时授权，并通过 `approval_instance_id=ELEVATION:<requestId>` 关联审计。后续接外部 BPM
  时可替换审批编排，不改变快照安全语义。
