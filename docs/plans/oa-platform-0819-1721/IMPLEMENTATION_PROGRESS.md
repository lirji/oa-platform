...
| **3** | PC 前端底座 `oa-console` | ✅ **完成，发布关卡 14/14、E2E 50/50** |
| **4** | 工作台 + 审批底座 | ✅ **完成，冒烟 25/25 全绿** |
| **4b** | ★ 接真中台（原 Phase 4 的阻塞项） | ✅ **完成，冒烟 28/28 全绿** |
| **5** | 考勤域 | ✅ **完成，冒烟 19/19 全绿** |
| **6** | 通知（后端） | ✅ **完成，冒烟 28/28 全绿** |
| **6** | 移动端 H5 `oa-mobile` | ✅ **完成，发布关卡 13/13** |
| **7** | 文档与行政 | ✅ **完成，冒烟 45/45 全绿** |
| **8** | 报表 / 审计 / 跑批 / 文件 / 渗透 | ✅ **完成，冒烟 40/40 全绿** |
| **8** | 全链路压测 + Nginx 前端交付 | ✅ **完成，6 场景 1000/1000、0 失败** |

> 冒烟总计 **231 条断言全绿**（14+28+32+25+28+19+28+45+40）。
> **后端、PC、移动端、JWT/WebSocket、CI 与发布门禁均已完成。**
> 最终全链路压测（1000 请求、并发 20）：6 个读路径均 0 失败；工作台 P99 63.3 ms，
> 通讯录 185.9 ms，组织树 12.1 ms，驾驶舱 205.2 ms，公告 48.7 ms，全部低于预算。
> 完整证据见 `docs/delivery/oa-platform-completion/`。
...
2. ~~Casdoor SSO 接入 + 权限渲染体系~~ → ✅ 权限体系、JWT 与本地 Casdoor 联调均已完成
3. ~~组织树 / 员工 / 角色授权页~~ → 已完成
...
## Phase 3 历史实施记录（截至 2026-08-20，当时未收口）—— PC 控制台 `oa-console`

> 本节保留当时的缺口快照用于追溯，**不代表当前状态**；最终状态见文末“交付收口”。
...
| ~~2~~ | ~~OpenAPI TS 契约未生成~~ | ✅ 已生成 88-path OpenAPI TS 契约并加漂移门禁 |
...
| ~~4~~ | ~~单测 2/9~~ | ✅ **8/9**（`271c72f`，共 **102 条**）。只剩第 9 条（授权/撤权/组织移动后的失效正确性）—— 它要真的发生一次撤权，更适合放进 e2e |
| ~~5~~ | ~~E2E 尚未执行~~ | ✅ 9 specs / 47 tests，A1–A10 全部自动化 |
| ~~6~~ | ~~知识库页未做~~ | ✅ `/kb` 对象授权页面与 E2E 已完成 |
| ~~7~~ | ~~step 14 未收口~~ | ✅ compose、镜像、Nginx、Phase 3 14/14 已完成 |
| ~~8~~ | ~~step 15 JWT 切换~~ | ✅ Casdoor/JWT/WS Phase 4 13/13；传统冒烟显式 DEV |

**建议顺序**：~~1+3~~ ✅ → ~~4~~ ✅ → **5（e2e，五条硬验收全靠它）** → 6 → 7 → 8。
...
### 补单测时抓出的三个真缺陷（2026-08-20 · `271c72f`）

不是"缺测试"，是代码本身的问题 —— 三个都不报错，只是安静地做错事。

1. **没有一个 queryKey 带 `permVersion`**。列表数据是后端按调用者的 `@DataScope`
   过滤出来的，缓存里那份**归属于某一个权限版本**；而权限变化时只失效了
   `PERM_KEY`/`ELEVATION_KEY`。结果是撤权之后**按钮消失了、行还在**。
   这正落在静默失败清单第 16 条的阴影里：少给行不报错，多给行同样不报错。

   修法是**把版本放进 key**而不是 invalidate：invalidate 会保留旧数据直到新数据回来，
   收权后的那几百毫秒用户看到的还是他已无权看的行；换 key 的中间态是骨架屏。
   新增 `usePermVersion()`，13 处查询带上，并加元测试守住下一个新增的查询。

2. **401 的 `giveUp` 没单飞**：`renew` 做了单飞，`giveUp` 没有 —— 5 个并发 401
   就是 5 次 `signinRedirect()`。"只续期一次"容易想到，"只跳一次登录"经常漏。

3. **IndexedDB 连接从不 close**：每次同步多留一个连接，既泄漏，又会让将来的
   `DB_VERSION` 升级被 blocked —— 升级事务根本不触发，
   "新版 schema 的代码跑在旧 schema 的库上"。
   这个 bug 是被测试逼出来的：`deleteDatabase` 被开着的连接 block，后续 `open`
   跟着挂死，表现为除第一条外全部超时。

**测试基础设施上的两个坑**：antd 会给恰好两个汉字的按钮文案中间插空格
（`insertSpace`），`getByRole({name:'撤权'})` 找不到"撤 权"；A10 与 queryKey
两条元测试只扫 UI 源码 —— 测试需要引用不存在的 code 来证明"未知 code 判拒"。
两条元测试都做了负向验证（写错 code → 红），未验证过能红的门禁不算门禁。

---

## 交付收口 ✅（2026-08-21）

按 PC → 契约/知识库 → 交付/CI → JWT/WS → 移动端 → 最终 QA 的顺序完成：

- PC A1–A10 已自动化，Vitest 105 条、Playwright 47 条；知识库 `/kb` 覆盖列表、搜索、详情复核、访问解释与 403。
- 主应用 88 条 OpenAPI path 已固定快照并生成 TS 类型，`gen:api:check` 参与 CI 漂移门禁。
- PC `:8404` 与移动 `:8405` 已进入 compose `apps` profile，均有生产 Nginx、SPA fallback、四上游代理、镜像与体积门禁。
- GitHub Actions 覆盖 JDK 21 Maven，以及 PC/移动端 frozen install、单测、构建和 gzip 预算。
- 四服务生产默认 JWT；本地 Casdoor OA SPA 可幂等注册，已实测签名、issuer、audience、PKCE、回调、续期和 8KB+ token。
- 浏览器通知使用 Redis 30 秒一次性 WS ticket，取用即删、严格 Origin，长期 token 不进入 URL；并发会话名额用 CAS 原子预留。
- 移动端完成待办审批、打卡、万人通讯录、公告查看/已读，覆盖 390×844 与 320px。
- 最终全链路读压测 6 场景各 1000 请求、并发 20、0 失败，P99 全部低于预算。

交付过程修复了 LOCAL 流程重启 ID 碰撞、首待办过早投影、E2E CORS 假绿、WS 会话上限竞态、
公告列表嵌套 JDBC 导致的连接池自锁，以及组织树压测参数写错导致的性能误判。

仍属明确边界的项目：JIT 提权接 BPMN、SpiceDB 知识库适配器、
真实外部消息供应商、排班/加班调休；生产域名、密钥、TLS、调度和目标环境容量验证由部署方完成。

## 权限增强：USER_GROUP + ABAC ✅（2026-08-21）

- V14 新增租户隔离的显式用户组与成员时间窗；组授权进入角色继承、数据范围、快照 TTL、缓存失效与来源解释。
- ABAC 条件按租户/角色/权限点管理，受限 SpEL 只读方法参数与用户上下文；同来源 AND、跨来源 OR、无条件旁路、错误 fail-closed。
- PC `/iam` 已升级为主体授权、用户组、ABAC 三视图，OpenAPI 为 96 paths；Phase 9 API 冒烟和 390px Playwright 通过。
- ABAC 仍默认关闭，使用 `OA_IAM_ABAC_ENABLED=true` 按环境灰度；关闭开关即可回退历史 RBAC。
