...
权限目录支持 `MENU`、`BUTTON`、`API`、`DATA`、`FIELD` 五种类型。当前数据库有 66 个权限点，
其中 9 个 `MENU`、2 个 `FIELD`、55 个 `API`；目录中的 `resource` 和 `method` 只是运营元数据，
...
| JIT 提权 | 临时激活高危能力 | 只能激活本人已有的直接永久角色授权，不能凭空加权 |
...
- 四份 `api-surface.golden` 冻结 135 个端点的“HTTP 方法 + 路径 + 权限点/公开立场”。
...
| `CUSTOM` | 指定组织路径前缀；前缀过多时退化为组织 ID 集合 |
| `NONE` | `1 = 0`，一行都不给 |
...
1. “主体授权”：选择 `USER`、`ORG_UNIT`、`POSITION` 或 `USER_GROUP`，选择已有角色和数据范围后授权；
   留空角色可只查询该主体。查询授权需要 `oa:iam:view`，新增和撤销分别需要
   `oa:iam:grant`、`oa:iam:revoke`。
2. “用户组”：拥有 `oa:iam:admin` 后可创建、改名、启停组，批量加入最多 500 个用户并设置成员时间窗。
3. “ABAC 条件”：拥有 `oa:iam:admin` 后选择角色和权限点，填写、校验、启停或软删除表达式。
4. “权限沙盘” `/iam/sandbox`：用 `why`、`explain` 和 `preview` 检查某用户的权限来源、范围和缓存真值。

当前管理端和 API 只读角色/权限目录，不提供角色、角色继承、角色权限矩阵的在线增删改。需要调整这些
结构时，应提交受评审的 Flyway 迁移和测试，并在迁移中推进 `perm_epoch`，不要直接在生产库改表。
若未来增加在线角色编辑，写服务必须同时维护继承闭包、校验环路、推进全局权限纪元并写审计。

`CUSTOM` 范围后端已支持 `scopeOrgIds`，但当前 PC “主体授权”表单只提供
`ALL/ORG_AND_SUB/ORG/SELF/NONE`；配置 `CUSTOM` 需使用授权 API，并显式传组织 ID 列表。
